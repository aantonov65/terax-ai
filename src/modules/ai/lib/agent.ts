...
const TOOL_LABELS: Record<string, (input: Record<string, unknown>) => string> = {
  ...
  create_product_from_config: () => "Creating product",
  run_product_research: () => "Running product research",
  save_strategy_plan: () => "Saving creative direction",
  build_strategy_json: () => "Building strategy.json",
  set_autonomous_mode: () => "Toggling autonomous mode",
  ...
  list_lfs_artifacts: () => "Listing LFS artifacts",
  read_lfs_artifact: (i) => `Reading ${shortPath(i.artifact_id)}`,
  edit_lfs_artifact: (i) => `Editing ${shortPath(i.artifact_id)}`,
  rerun_lfs_checks: (i) => `Rerunning ${String(i.mode ?? "LFS")} checks`,
  retry_lfs_failures: (i) => `Retrying ${String(i.stage ?? "failed LFS stage")}`,
  export_lfs: () => "Exporting LFS scripts",
  cancel_lfs_job: () => "Canceling LFS job",
  ...
  generate_images: (i) => `Generating images for ${String(i.batch_id ?? "batch")}`,
  run_lfs_and_images: (i) => `Running LFS + images for ${String(i.batch_id ?? "batch")}`,
  get_batch_status: (i) => `Inspecting ${String(i.batch_id ?? "batch")}`,
  open_artifact: (i) => `Opening ${shortPath(i.path)}`,
  todo_write: (i) =>
